...
@RestController("marryStateController")
@RequestMapping("/system/marryState")
public class MarryStateController {
	@Autowired
	private MarryStateTransport marryStateTransport;
	...
	public ResponseVO getPageListByQuery(@PathVariable("pageNum") Integer pageNum,
	                                     ...
	                                     @RequestBody MarryState query) throws Exception{
		// 根据参数生成相应的 QueryVO 对象
		PageVO<MarryState> pageVO = new PageVO<MarryState>(pageNum,pageSize);
		PageQueryVO<MarryState> queryVO = new PageQueryVO<>();
		queryVO.setQuery(query);
		queryVO.setPageVO(pageVO);
		// 进行分页查询
		pageVO = marryStateTransport.getPageListByQuery(queryVO);
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody MarryState query) throws Exception{
		return  ResponseVO.getSuccessResponseVO(marryStateTransport.getListByQuery(query));
	}
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO  save(@RequestBody MarryState entity) throws Exception{
		// 先判断提交的 code 是否符合要求
		String code = entity.getCode();

		if(code != null && !"".equals(code)){
			// 此时 code 有值，判断是否已被使用
			MarryState idType = marryStateTransport.getByCode(code);
			if(idType != null){
				return ResponseVO.getErrorResponseVO("该编码已被使用，请重新输入");
			} else {
				if(marryStateTransport.save(entity)){
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody MarryState entity) throws  Exception{
		// 判断 code 是否合格
		String code = entity.getCode();
		if(code != null && !"".equals(code)){
			MarryState idType = marryStateTransport.getByCode(code);
			...
			// 此时可以提交修改
			if(marryStateTransport.update(entity)){
	...
	public ResponseVO getById(@PathVariable("id") String id) throws Exception{
		return ResponseVO.getSuccessResponseVO(marryStateTransport.getById(id));
	}
	...
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception{
		return ResponseVO.getSuccessResponseVO(marryStateTransport.getByCode(code));
