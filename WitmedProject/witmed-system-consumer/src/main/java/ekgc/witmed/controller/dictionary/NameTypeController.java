...
@RestController("nameTypeController")
@RequestMapping("/system/nameType")
public class NameTypeController {
	@Autowired
	private NameTypeTransport nameTypeTransport;
	...
	public ResponseVO getPageListByQuery(@PathVariable("pageNum") Integer pageNum,
	                                     ...
	                                     @RequestBody NameType query) throws Exception{
		// 根据参数生成相应的 QueryVO 对象
		PageVO<NameType> pageVO = new PageVO<NameType>(pageNum,pageSize);
		PageQueryVO<NameType> queryVO = new PageQueryVO<>();
		queryVO.setQuery(query);
		queryVO.setPageVO(pageVO);
		// 进行分页查询
		pageVO = nameTypeTransport.getPageListByQuery(queryVO);
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody NameType query) throws Exception{
		return  ResponseVO.getSuccessResponseVO(nameTypeTransport.getListByQuery(query));
	}
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO  save(@RequestBody NameType entity) throws Exception{
		// 先判断提交的 code 是否符合要求
		String code = entity.getCode();

		if(code != null && !"".equals(code)){
			// 此时 code 有值，判断是否已被使用
			NameType NameType = nameTypeTransport.getByCode(code);
			if(NameType != null){
				return ResponseVO.getErrorResponseVO("该编码已被使用，请重新输入");
			} else {
				if(nameTypeTransport.save(entity)){
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody NameType entity) throws  Exception{
		// 判断 code 是否合格
		String code = entity.getCode();
		if(code != null && !"".equals(code)){
			NameType NameType = nameTypeTransport.getByCode(code);
			if(NameType != null ){
				if(!NameType.getId().equals(entity.getId())){
					// 此时表示，修改后提交的信息中编码已被使用了
					return ResponseVO.getErrorResponseVO("该编码已被占用");
				}
			}
			// 此时可以提交修改
			if(nameTypeTransport.update(entity)){
	...
	public ResponseVO getById(@PathVariable("id") String id) throws Exception{
		return ResponseVO.getSuccessResponseVO(nameTypeTransport.getById(id));
	}
	...
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception{
		return ResponseVO.getSuccessResponseVO(nameTypeTransport.getByCode(code));
