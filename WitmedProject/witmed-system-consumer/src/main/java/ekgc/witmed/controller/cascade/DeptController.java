...
public class DeptController {
	@Autowired
	private DeptTransport deptTransport;
	...
	public ResponseVO getPageListByQuery(@PathVariable("pageNum") Integer pageNum,
		...
		// 进行分页查询
		pageVO = deptTransport.getPageListByQuery(queryVO);
	...
	public ResponseVO getListByQuery(@RequestBody CascadeEntity query) throws Exception{
		return  ResponseVO.getSuccessResponseVO(deptTransport.getListByQuery(query));
	}
	...
	public ResponseVO  save(@RequestBody CascadeEntity entity) throws Exception{
		...
		if(code != null && !"".equals(code)){
			// 此时 code 有值，判断是否已被使用
			CascadeEntity idType = deptTransport.getByCode(code);
			if(idType != null){
				return ResponseVO.getErrorResponseVO("该编码已被使用，请重新输入");
			} else {
				if(deptTransport.save(entity)){
	...
	public ResponseVO update(@RequestBody CascadeEntity entity) throws  Exception{
		...
		if(code != null && !"".equals(code)){
			...
			// 此时可以提交修改
			if(deptTransport.update(entity)){
	...
	public ResponseVO getById(@PathVariable("id") String id) throws Exception{
		return ResponseVO.getSuccessResponseVO(deptTransport.getById(id));
	}
	...
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception{
		return ResponseVO.getSuccessResponseVO(deptTransport.getByCode(code));
