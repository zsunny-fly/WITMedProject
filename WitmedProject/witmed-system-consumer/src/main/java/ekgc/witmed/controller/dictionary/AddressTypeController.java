...
public class AddressTypeController {
	@Autowired
	private AddressTypeTransport addressTypeTransport;
	...
	public ResponseVO getPageListByQuery(@PathVariable("pageNum") Integer pageNum,
		...
		PageQueryVO<AddressType> queryVO = new PageQueryVO<>();
		queryVO.setQuery(query);
		queryVO.setPageVO(pageVO);
		// 进行分页查询
		pageVO = addressTypeTransport.getPageListByQuery(queryVO);
	...
	public ResponseVO getListByQuery(@RequestBody AddressType query) throws Exception{
		return  ResponseVO.getSuccessResponseVO(addressTypeTransport.getListByQuery(query));
	}
	...
	public ResponseVO  save(@RequestBody AddressType entity) throws Exception{
		...
		if(code != null && !"".equals(code)){
			// 此时 code 有值，判断是否已被使用
			AddressType idType = addressTypeTransport.getByCode(code);
			if(idType != null){
				return ResponseVO.getErrorResponseVO("该编码已被使用，请重新输入");
			} else {
				if(addressTypeTransport.save(entity)){
	...
	public ResponseVO update(@RequestBody AddressType entity) throws  Exception{
		...
		if(code != null && !"".equals(code)){
			...
			// 此时可以提交修改
			if(addressTypeTransport.update(entity)){
	...
	public ResponseVO getById(@PathVariable("id") String id) throws Exception{
		return ResponseVO.getSuccessResponseVO(addressTypeTransport.getById(id));
	}
	...
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception{
		return ResponseVO.getSuccessResponseVO(addressTypeTransport.getByCode(code));
