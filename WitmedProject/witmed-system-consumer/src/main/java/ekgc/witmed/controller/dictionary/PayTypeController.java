...
import ekgc.witmed.transport.dictionary.PayTypeTransport;
...
public class PayTypeController {
	...
	@Autowired
	private PayTypeTransport payTypeTransport;
	...
	public ResponseVO getPageListByQuery(@PathVariable("pageNum") Integer pageNum,
		...
		PageQueryVO<PayType> queryVO = new PageQueryVO<>();
		queryVO.setQuery(query);
		queryVO.setPageVO(pageVO);
		// 进行分页查询
		pageVO = payTypeTransport.getPageListByQuery(queryVO);
	...
	public ResponseVO getListByQuery(@RequestBody PayType query) throws Exception{
		return  ResponseVO.getSuccessResponseVO(payTypeTransport.getListByQuery(query));
	}
	...
	public ResponseVO  save(@RequestBody PayType entity) throws Exception{
		...
		if(code != null && !"".equals(code)){
			// 此时 code 有值，判断是否已被使用
			PayType idType = payTypeTransport.getByCode(code);
			if(idType != null){
				return ResponseVO.getErrorResponseVO("该编码已被使用，请重新输入");
			} else {
				if(payTypeTransport.save(entity)){
	...
	public ResponseVO update(@RequestBody PayType entity) throws  Exception{
		...
		if(code != null && !"".equals(code)){
			...
			// 此时可以提交修改
			if(payTypeTransport.update(entity)){
	...
	public ResponseVO getById(@PathVariable("id") String id) throws Exception{
		return ResponseVO.getSuccessResponseVO(payTypeTransport.getById(id));
	}
	...
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception{
		return ResponseVO.getSuccessResponseVO(payTypeTransport.getByCode(code));
