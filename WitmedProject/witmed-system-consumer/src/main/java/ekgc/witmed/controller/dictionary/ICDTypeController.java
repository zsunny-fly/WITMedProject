...
@RestController("iCDTypeController")
@RequestMapping("/system/icdType")
public class ICDTypeController extends BaseController {
	@Autowired
	private ICDTypeTransport iCDTypeTransport;
	...
	public ResponseVO getPageListByQuery(@PathVariable("pageNum") Integer pageNum,
	                                     ...
	                                     @RequestBody ICDType query) throws Exception{
		// 根据参数生成相应的 QueryVO 对象
		PageVO<ICDType> pageVO = new PageVO<ICDType>(pageNum,pageSize);
		PageQueryVO<ICDType> queryVO = new PageQueryVO<>();
		queryVO.setQuery(query);
		queryVO.setPageVO(pageVO);
		// 进行分页查询
		pageVO = iCDTypeTransport.getPageListByQuery(queryVO);
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody ICDType query) throws Exception{
		return  ResponseVO.getSuccessResponseVO(iCDTypeTransport.getListByQuery(query));
	}
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO  save(@RequestBody ICDType entity) throws Exception{
		// 先判断提交的 code 是否符合要求
		String code = entity.getCode();

		if(code != null && !"".equals(code)){
			// 此时 code 有值，判断是否已被使用
			ICDType ICDType = iCDTypeTransport.getByCode(code);
			if(ICDType != null){
				return ResponseVO.getErrorResponseVO("该编码已被使用，请重新输入");
			} else {
				if(iCDTypeTransport.save(entity)){
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody ICDType entity) throws  Exception{
		// 判断 code 是否合格
		String code = entity.getCode();
		if(code != null && !"".equals(code)){
			ICDType ICDType = iCDTypeTransport.getByCode(code);
			if(ICDType != null ){
				if(!ICDType.getId().equals(entity.getId())){
					// 此时表示，修改后提交的信息中编码已被使用了
					return ResponseVO.getErrorResponseVO("该编码已被占用");
				}
			}
			// 此时可以提交修改
			if(iCDTypeTransport.update(entity)){
	...
	public ResponseVO getById(@PathVariable("id") String id) throws Exception{
		return ResponseVO.getSuccessResponseVO(iCDTypeTransport.getById(id));
	}
	...
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception{
		return ResponseVO.getSuccessResponseVO(iCDTypeTransport.getByCode(code));
