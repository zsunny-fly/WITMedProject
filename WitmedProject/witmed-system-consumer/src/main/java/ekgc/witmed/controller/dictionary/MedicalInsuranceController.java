...
@RequestMapping("/system/medicalInsurance")
public class MedicalInsuranceController {


	@Autowired
	private MedicalInsuranceTransport medicalInsuranceTransport;
	...
	public ResponseVO getPageListByQuery(@PathVariable("pageNum") Integer pageNum,
		...
		PageQueryVO<MedicalInsurance> queryVO = new PageQueryVO<MedicalInsurance>();
		queryVO.setQuery(query);
		queryVO.setPageVO(pageVO);
		// 进行分页查询
		pageVO = medicalInsuranceTransport.getPageListByQuery(queryVO);
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody MedicalInsurance query) throws Exception{
		return  ResponseVO.getSuccessResponseVO(medicalInsuranceTransport.getListByQuery(query));
	}
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO  save(@RequestBody MedicalInsurance entity) throws Exception{
		// 先判断提交的 code 是否符合要求
		String code = entity.getCode();

		if(code != null && !"".equals(code)){
			// 此时 code 有值，判断是否已被使用
			MedicalInsurance idType = medicalInsuranceTransport.getByCode(code);
			if(idType != null){
				return ResponseVO.getErrorResponseVO("该编码已被使用，请重新输入");
			} else {
				if(medicalInsuranceTransport.save(entity)){
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody MedicalInsurance entity) throws  Exception{
		// 判断 code 是否合格
		String code = entity.getCode();
		if(code != null && !"".equals(code)){
			MedicalInsurance idType = medicalInsuranceTransport.getByCode(code);
			...
			// 此时可以提交修改
			if(medicalInsuranceTransport.update(entity)){
	...
	public ResponseVO getById(@PathVariable("id") String id) throws Exception{
		return ResponseVO.getSuccessResponseVO(medicalInsuranceTransport.getById(id));
	}
	...
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception{
		return ResponseVO.getSuccessResponseVO(medicalInsuranceTransport.getByCode(code));
