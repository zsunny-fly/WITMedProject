...
public class IdTypeController extends BaseController {
	@Autowired
	private IdTypeTransport idTypeTransport;
	...
	public ResponseVO getPageListByQuery(@PathVariable("pageNum") Integer pageNum,
		...
		PageQueryVO<IdType> queryVO = new PageQueryVO<>();
		queryVO.setQuery(query);
		queryVO.setPageVO(pageVO);
		// 进行分页查询
		pageVO = idTypeTransport.getPageListByQuery(queryVO);
	...
	public ResponseVO getListByQuery(@RequestBody IdType query) throws Exception{
		return  ResponseVO.getSuccessResponseVO(idTypeTransport.getListByQuery(query));
	}
	...
	public ResponseVO  save(@RequestBody IdType entity) throws Exception{
		...
		if(code != null && !"".equals(code)){
			// 此时 code 有值，判断是否已被使用
			IdType idType = idTypeTransport.getByCode(code);
			if(idType != null){
				return ResponseVO.getErrorResponseVO("该编码已被使用，请重新输入");
			} else {
				if(idTypeTransport.save(entity)){
	...
	public ResponseVO update(@RequestBody IdType entity) throws  Exception{
		...
		if(code != null && !"".equals(code)){
			...
			// 此时可以提交修改
			if(idTypeTransport.update(entity)){
	...
	public ResponseVO getById(@PathVariable("id") String id) throws Exception{
		return ResponseVO.getSuccessResponseVO(idTypeTransport.getById(id));
	}
	...
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception{
		return ResponseVO.getSuccessResponseVO(idTypeTransport.getByCode(code));
