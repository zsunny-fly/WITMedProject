...
@RequestMapping("/system/priceType")
public class PriceTypeController {

	@Autowired
	private PriceTypeTransport priceTypeTransport;
	...
	public ResponseVO getPageListByQuery(@PathVariable("pageNum") Integer pageNum,
	                                     ...
	                                     @RequestBody PriceType query) throws Exception{
		// 根据参数生成相应的 QueryVO 对象
		PageVO<PriceType> pageVO = new PageVO<PriceType>(pageNum,pageSize);
		PageQueryVO<PriceType> queryVO = new PageQueryVO<PriceType>();
		queryVO.setQuery(query);
		queryVO.setPageVO(pageVO);
		// 进行分页查询
		pageVO = priceTypeTransport.getPageListByQuery(queryVO);
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody PriceType query) throws Exception{
		return  ResponseVO.getSuccessResponseVO(priceTypeTransport.getListByQuery(query));
	}
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO  save(@RequestBody PriceType entity) throws Exception{
		// 先判断提交的 code 是否符合要求
		String code = entity.getCode();

		if(code != null && !"".equals(code)){
			// 此时 code 有值，判断是否已被使用
			PriceType idType = priceTypeTransport.getByCode(code);
			if(idType != null){
				return ResponseVO.getErrorResponseVO("该编码已被使用，请重新输入");
			} else {
				if(priceTypeTransport.save(entity)){
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody PriceType entity) throws  Exception{
		// 判断 code 是否合格
		String code = entity.getCode();
		if(code != null && !"".equals(code)){
			PriceType idType = priceTypeTransport.getByCode(code);
			...
			// 此时可以提交修改
			if(priceTypeTransport.update(entity)){
	...
	public ResponseVO getById(@PathVariable("id") String id) throws Exception{
		return ResponseVO.getSuccessResponseVO(priceTypeTransport.getById(id));
	}
	...
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception{
		return ResponseVO.getSuccessResponseVO(priceTypeTransport.getByCode(code));
