...
@RequestMapping("/system/genderType")
public class GenderTypeController {
	@Autowired
	private GenderTypeTransport genderTypeTransport;
	...
	public ResponseVO getPageListByQuery(@PathVariable("pageNum") Integer pageNum,
	                                     ...
	                                     @RequestBody DictionaryEntity query) throws Exception{
		// 根据参数生成相应的 QueryVO 对象
		PageVO<DictionaryEntity> pageVO = new PageVO<DictionaryEntity>(pageNum,pageSize);
		PageQueryVO<DictionaryEntity> queryVO = new PageQueryVO<>();
		queryVO.setQuery(query);
		queryVO.setPageVO(pageVO);
		// 进行分页查询
		pageVO = genderTypeTransport.getPageListByQuery(queryVO);
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody DictionaryEntity query) throws Exception{
		return  ResponseVO.getSuccessResponseVO(genderTypeTransport.getListByQuery(query));
	}
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO  save(@RequestBody DictionaryEntity entity) throws Exception{
		// 先判断提交的 code 是否符合要求
		String code = entity.getCode();

		if(code != null && !"".equals(code)){
			// 此时 code 有值，判断是否已被使用
			DictionaryEntity idType = genderTypeTransport.getByCode(code);
			if(idType != null){
				return ResponseVO.getErrorResponseVO("该编码已被使用，请重新输入");
			} else {
				if(genderTypeTransport.save(entity)){
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody DictionaryEntity entity) throws  Exception{
		// 判断 code 是否合格
		String code = entity.getCode();
		if(code != null && !"".equals(code)){
			DictionaryEntity idType = genderTypeTransport.getByCode(code);
			...
			// 此时可以提交修改
			if(genderTypeTransport.update(entity)){
	...
	public ResponseVO getById(@PathVariable("id") String id) throws Exception{
		return ResponseVO.getSuccessResponseVO(genderTypeTransport.getById(id));
	}
	...
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception{
		return ResponseVO.getSuccessResponseVO(genderTypeTransport.getByCode(code));
