...
public class TokenUtil {
	// 设定生成 Token 的加密算法
	private static Algorithm algorithm = Algorithm.HMAC256(ConstantUtil.TOKEN_SECRET_KEY);
	...
	public static String createToken(String id) {
		...
		Long expireTime = System.currentTimeMillis() + ConstantUtil.TOKEN_EXPIRE_MILLIS;
	...
	public static String verifyToken(String token) {
		...
		if (token != null && !"".equals(token.trim())) {
			if(token.replace(".","-").split("-").length == 3){
				// 此时 Token 格式有效
